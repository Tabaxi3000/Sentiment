...
from ensemble_models import VotingEnsemble, StackingEnsemble
...
def load_model(model_type, model_path=None):
    ...
    Args:
        model_type (str): Type of model to load ('lr', 'svm', 'adaboost', 'lstm', 'bert', 'roberta', 'voting', 'stacking')
    ...
    default_paths = {
        ...
        'bert': 'models/bert_sentiment.pt',
        'roberta': 'models/roberta_sentiment.pt',
    ...
    if model_type == 'lr':
        ...
        model = SentimentBERT(use_pretrained=True)
        model.load_model(model_path)
    elif model_type == 'roberta':
        model = SentimentRoBERTa(use_pretrained=True)
        model.load_model(model_path)
...
def interactive_mode(model):
    ...
    Args:
        model: Sentiment model
    """
    print("\nInteractive Sentiment Prediction Mode")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Sentiment Prediction Script")
    
    parser.add_argument(
        "--model",
        type=str,
        default="svm",
        choices=["lr", "svm", "adaboost", "lstm", "bert", "roberta", "voting", "stacking"],
